...
static void emit_stmt(Bytecode* bc, Chunk* c, EmitCtx* ec, const Stmt* st) {
    switch (st->kind) {
        case ST_LET: {
            ...
            // IMPORTANT: DO NOT OP_POP here. OP_SET_LOCAL consumes value and does not push.
        } break;
        ...
        case ST_SLET: {
            // Hoist to nearest active loop: first time declares, later acts like SET.
            if (ec->loop_depth == 0) {
            ...
            if (!loop_has_name(ec, st->as.let_stmt.name, &slot)) {
                // create binding now (even if inside nested blocks, we keep it alive via scope_pop loop check)
                uint16_t dummy=0;
                if (map_get(c, st->as.let_stmt.name, &dummy)) {
                    fprintf(stderr, "TypeError: attempted to redeclare '%s' (use VOID/VOIDLET)\n", st->as.let_stmt.name);
                    exit(1);
        ...
        case ST_SET: {
            ...
            // compile op on two stack values (a, b)->result
            auto emit_math = [&](AssignOp opkind) {
                switch (opkind) {
